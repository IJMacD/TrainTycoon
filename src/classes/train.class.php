...
class Train
{
	...
	{
		return $this->name && strlen($this->name) ? $this->name : "Train ".$this->id;
	}
	...
	function getPreviousDestinationID()
	{
		$index = $this->segment < 0
			? count($this->route) + $this->segment
			: $this->segment - 1;

		$station = $this->route[$index];

		return $station['station_id'];
	}

	function getPreviousTrackNodeID()
	{
		if (!$this->track) {
			die($this->getName()." not on any track");
		}
	...
	function getNearestStationID () {
		...
		if ($this->getProgress() < 50) {
			return $track['from_station_id'];
		}
		return $track['to_station_id'];
	}
	...
	function setNextDestination()
	{
		...
		// $g->insertLog("set new destination of " . $this->getNextDestination() . " for " . $this->getName());
	}
	...
	function moveToTrack ($track_id, $direction) {
		...
		$g->updateTrain($this->id, 'progress', $this->direction > 0 ? 1 : 99);
	}
	...
	static function getTrain ($id)
	{
		...
		if(!isset(self::$_singleton[$id]))
		{
			...
			$is_at_next_station = $is_at_node
				&& $train->getNextTrackNodeID() == $train->getNextDestinationID();

			// echo "direction: $train->direction progress: $train->progress<br>\n";
			// echo "is_at_node: $is_at_node is_at_next_station: $is_at_next_station<br>\n";
			...
			// echo "state: $train->state<br>\n";

			for($i = 1; $i <= 8; $i++)
			{
				if($t['Car_'.$i] && strlen($t['Car_'.$i]))
					$train->cars[] = $t['Car_'.$i];
			}
